...
beforeEach(() => {
  vi.clearAllMocks();
  api.listPendingApprovals.mockResolvedValue([mockApproval]);
  api.listConnections.mockResolvedValue([]);
  api.getEvent.mockResolvedValue(mockEvent);
  api.getEventTimeline.mockResolvedValue({ items: [], next_cursor: null });
  api.listEventAttachments.mockResolvedValue([]);
  api.getPlan.mockResolvedValue(mockPlan);
  ...
  api.getPreferences.mockResolvedValue({
    autonomy_level: "safe_actions",
    category_behaviors: {},
    daily_message_cap: 10,
    daily_calendar_cap: 10,
  });
});
