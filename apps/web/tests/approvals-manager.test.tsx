...
const approval = {
  ...
  action: {
    id: "action-1",
    action_type: "travel.send_family_message",
    connector: "telegram",
    input: { message: "ready" },
    status: "waiting_approval",
    risk_level: "red",
    requires_approval: true,
    policy_reason: "red_requires_approval",
    completed_at: null,
  },
};
...
describe("ApprovalsManager", () => {
  ...
  it("shows action context and requires confirmation before a high-risk approval", async () => {
    ...
    expect(await screen.findByText("send family message")).toBeInTheDocument();
    expect(screen.getByText("High risk")).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "Allow" }));
    expect(
      screen.getByRole("dialog", { name: "Confirm this high-risk action" }),
    ).toBeInTheDocument();
    ...
    expect(screen.queryByText("send family message")).not.toBeInTheDocument();
  });
});
