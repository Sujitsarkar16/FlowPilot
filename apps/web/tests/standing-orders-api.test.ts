...
const order = { id: "rule-1", instruction: "Prepare flights", compiled_rule: null, version: 1, enabled: false, compilation_status: "pending", last_error: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" };
...
describe("standing-order API client", () => {
  it("uses the expected CRUD and simulation routes", async () => {
    const fetchFn = vi.fn<typeof fetch>()
      ...
      .mockResolvedValueOnce(json({ event_type: "travel_booked", matched: true, proposed_actions: [], warnings: [] }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }));
    const client = createApiClient({ baseUrl: "https://api.example.test", fetchFn, getAccessToken: async () => "token" });
