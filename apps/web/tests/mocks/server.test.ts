...
describe("test API utilities", () => {
  it("serves API-client requests without a backend", async () => {
    const server = createMockServer({ "GET /api/v1/dashboard/summary": () => json({ events_today: 1 }) });
    const api = createApiClient({ baseUrl: "http://flowpilot.test", fetchFn: server.fetch, getAccessToken: async () => null });
  ...
  it("delivers deterministic SSE payloads", () => {
    const source = new MockEventSource("/events"); const seen: unknown[] = [];
