...
async def test_gmail_reader_parses_bounded_multipart_and_redacts_headers() -> None:
            ...
            json={
                ...
                "payload": {
                    ...
                    "parts": [
                        {
                            "mimeType": "text/html",
                            "body": {"data": encoded("<b>Ignored fallback</b>")},
                        },
                        {"mimeType": "text/plain", "body": {"data": encoded("Flight confirmed")}},
                        {
                            "filename": "ticket.pdf",
                            "mimeType": "application/pdf",
                            "body": {"size": 12, "attachmentId": "attachment-1"},
                        },
    ...
    assert message.as_source().attachments[0].model_dump() == {
        "name": "ticket.pdf",
...
async def test_gmail_refreshes_access_token_with_injected_transport() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200, json={"access_token": "fresh-access", "refresh_token": "fresh-refresh"}
        )
...
@pytest.mark.asyncio
async def test_gmail_uses_indexed_query_and_downloads_only_selected_attachment() -> None:
    pdf = b"%PDF-1.7 ticket"

    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/messages"):
            assert request.url.params["q"].startswith("newer_than:2m")
            assert request.url.params["maxResults"] == "25"
            return httpx.Response(200, json={"messages": [{"id": "message-1"}]})
        assert request.url.path.endswith("/messages/message-1/attachments/attachment-1")
        return httpx.Response(
            200,
            json={"data": base64.urlsafe_b64encode(pdf).decode().rstrip("=")},
        )

    client = GmailClient(transport=httpx.MockTransport(handler))
    page = await client.list_messages("access", query="newer_than:2m ticket", max_results=25)
